...
export function IMAlertsTable({
  ...
  return (
    <div className="w-full h-full relative">
      <Table className="w-full max-h-full border-collapse table-auto">
        <TableHeader className="sticky top-0 z-10 bg-[hsl(var(--secondary))]">
            ...
            >
              ...
              <TableCell className="font-mono text-sm text-muted-foreground">
                ...
                {(typeof row.baseline === "number"
                  ? row.baseline
                  : Number(row.baseline || (row.price || 0) - (row.change || 0))
                ).toFixed(2)}
              </TableCell>
            </TableRow>
          ))}
